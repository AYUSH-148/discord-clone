...
interface chatMessagesProps {
  ...
  paramValue: string; // channelId
  type: "channel" | "conversation"
}

const ChatMessages =
  ({ name, member, chatId, apiUrl,
    socketQuery, socketUrl, paramKey, paramValue, type }:
    chatMessagesProps) => {
    ...
    if (status === "error") {
      return (
        <div className='flex flex-col flex-1 justify-center items-center'>
          ...
          <p className='text-xs text-zinc-500 dark:text-zinc-400'>Somethig went wrong!</p>
        </div>
                ...
                {group.items.map((message: MessageWithMemberWithProfile) => (
                  <div key={message.id}>
                    <ChatItem
                      ...
                      memberHavingMsg={message.member}
                    />
                  </div>
                ))}
