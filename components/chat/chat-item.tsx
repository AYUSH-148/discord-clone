...
interface ChatItemProps {
    id: string;
    content: string;
    memberHavingMsg: Member & {    // member who does particular message
        profile: Profile;
    };
    timestamp: string;
    fileUrl?: string | null;
    deleted: boolean;
    currentMember: Member; // current logged in user
    isUpdated: boolean;
    socketUrl: string;
...
const ChatItem = ({
    id, content, memberHavingMsg, timestamp, fileUrl, deleted, isUpdated, socketUrl, socketQuery, currentMember
}: ChatItemProps) => {
    const params = useParams();
    const router = useRouter();

    const onMemberClick = ()=>{
        if(memberHavingMsg.id === currentMember.id){
            return;
        }
        router.push(`/servers/${params?.serverId}/conversation/${memberHavingMsg.id}`)
    }
   ...
   const onSubmit = async(values:z.infer<typeof formSchema>)=>{
    ...
    const isOwner = currentMember.id === memberHavingMsg.id;
    ...
    return (
        <div className='relative  group flex items-center hover:bg-black/5 p-4 transition w-full'>
            ...
            <div onClick={onMemberClick} className='cursor-pointer hover:drop-shadow-md transition mr-2'>
                <UserAvatar src={memberHavingMsg.profile.imageUrl} />
            </div>
            <div className='flex flex-col w-full'>
                <div className='flex items-center gap-x-2'>
                    <div className='flex items-center'>
                        <p onClick={onMemberClick} className='font-semibold textsm hover:underline cursor-pointer'>{memberHavingMsg.profile.name}</p>
                        <ActionTooltip label={memberHavingMsg.role}>
                            {roleIconMap[memberHavingMsg.role]}
                        </ActionTooltip>
                    </div>
                ...
                {!fileUrl && !isEditing && (
                    <p className={cn("text-sm text-zinc-600 dark:text-zinc-300 mt-1", deleted && "italic text-zinc-500 dark:text-zinc-400 text0sm mt-1")}>
                        {content}
                        {isUpdated && !deleted && (
                            <span className='text-[10px] mx-2 text-zinc-500 dark:text0zinc-400'>(edited)</span>
                        )}
                    </p>
                )}
                {!fileUrl && isEditing && (
